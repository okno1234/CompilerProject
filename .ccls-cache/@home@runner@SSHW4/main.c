...
void printing(token_type *lexeme, char** iden, int lexemeMax, char** intarr, char** substrarr, FILE *fp2, int skip[]);
int isSymbol(char c);
int isLet(char c);
int isNum(char c);
...
void proc_declaration(){
    while (atoi(tokenList[token]) == procsym) {
        token++;
        if (atoi(tokenList[token]) != identsym){
            printf("Error: procedure name must be declared\n");
            exit(0);
        } 
        token++;
        if (atoi(tokenList[token]) != semicolonsym){
            printf("Error: semicolon must follow procedure name\n");
            exit(0);
        } 
        token++;
        block();
        if (atoi(tokenList[token]) != semicolonsym){
            printf("Error: semicolon must end procedure\n");
            exit(0);
        } 
        token++;
    }
...
void statement() {
    if (atoi(tokenList[token]) == identsym){ //if identifer, get next token and check symbol table
        ...
        return;
    } else if (atoi(tokenList[token]) == callsym) {
        token++;
    if (atoi(tokenList[token]) != identsym) {
